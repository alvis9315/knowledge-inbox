...
- **禁止偷懶簡化(2026-07-14 方案同步事件)**:每個行為都要問「這個變更的
  正確處置是什麼」並各自做對,不得為省事挑一個統一便宜點。具體:
  雙儲存層(本機+雲端)的狀態,**每個變更動作必須明確歸類**——
  即時同步 / 確認點才同步(完成/取消語意)/ 僅本機(訪客/草稿,附原因註解)。
  沒歸類就實作 = 兩邊狀態不一致的 bug 溫床
