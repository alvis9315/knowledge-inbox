...
- [x] **多主題平台軟訊號**(H3):有擷取標題時 youtube/IG 等從硬鎖 domain 降為 +1,
  標題關鍵字跨 domain 競爭(AI 教學影片進學習不進社群)。✅ 2026-07-16(3d976b5)
- [x] **字典批次匯入**:網頁版 Claude 用分類樹產 JSON → 自學字典畫面「匯入字典」
  貼上合併(名稱→key 解析、通用平台過濾、同詞取大)。零 API 費的批次建典。✅ 2026-07-17(d97f9c1)
- [x] **書籤批次匯入**(齒輪選單):貼清單/書籤 HTML → hostname 分組,一組選一次
  分類=整組建檔+學會該站。存量一次清。✅ 2026-07-17(a027d01)
