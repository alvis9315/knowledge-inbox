...
import { ref } from 'vue'
import type { GridDistortionConfig } from './KnowledgeGridDistortion.vue'
import BgPresetTabs from '@/features/theme/BgPresetTabs.vue'

// KnowledgeGridDistortion 的控制面板(Hue/XOffset/Speed/Intensity/Size),
// 直接編輯傳入的 reactive config,調整即時生效。模式同 ThreadsControls。
const props = defineProps<{ config: GridDistortionConfig; closable?: boolean }>()
const emit = defineEmits<{ done: []; cancel: [] }>()

const sliders = [
  { key: 'grid', label: 'Grid', min: 6, max: 40, step: 1, digits: 0 },
  { key: 'mouse', label: 'Radius', min: 0.05, max: 0.4, step: 0.01, digits: 2 },
  { key: 'strength', label: 'Strength', min: 0.05, max: 0.5, step: 0.01, digits: 2 },
  { key: 'relaxation', label: 'Relaxation', min: 0.7, max: 0.98, step: 0.01, digits: 2 },
] as const

const copied = ref(false)
const copyProps = () => {
  const c = props.config
  const text = `<KnowledgeGridDistortion
  :grid="${c.grid}"
  :mouse="${c.mouse}"
  :strength="${c.strength}"
...
<template>
  <div class="griddistortion-controls">
    <BgPresetTabs />
