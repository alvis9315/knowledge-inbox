...
import type { RadarConfig } from './KnowledgeRadar.vue'
import BgPresetTabs from '@/features/theme/BgPresetTabs.vue'

// KnowledgeRadar 的控制面板(Speed/Scale/Brightness/BandHeight/ColorSpeed
// + 雙色 + 互動),直接編輯傳入的 reactive config。模式同 ThreadsControls。
const props = defineProps<{ config: RadarConfig; closable?: boolean }>()
const emit = defineEmits<{ done: []; cancel: [] }>()

const COLOR_PRESETS = ['#9f29ff', '#61dca3', '#22d3ee', '#f4c430', '#fb7185', '#05060d']
const sliders = [
  { key: 'speed', label: 'Speed', min: 0, max: 3, step: 0.1, digits: 1 },
  { key: 'scale', label: 'Scale', min: 0.2, max: 2, step: 0.05, digits: 2 },
  { key: 'ringCount', label: 'Rings', min: 3, max: 24, step: 1, digits: 0 },
  { key: 'spokeCount', label: 'Spokes', min: 0, max: 24, step: 1, digits: 0 },
  { key: 'sweepSpeed', label: 'Sweep', min: 0, max: 3, step: 0.1, digits: 1 },
  { key: 'brightness', label: 'Brightness', min: 0.2, max: 2, step: 0.05, digits: 2 },
] as const
const colors = [
  { key: 'color', label: '雷達色' },
...
const copyProps = () => {
  ...
  const text = `<KnowledgeRadar
  :speed="${c.speed}"
  :scale="${c.scale}"
  :ring-count="${c.ringCount}"
  :spoke-count="${c.spokeCount}"
  :sweep-speed="${c.sweepSpeed}"
