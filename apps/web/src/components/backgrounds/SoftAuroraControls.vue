...
const props = defineProps<{ config: SoftAuroraConfig; closable?: boolean }>()
const emit = defineEmits<{ done: []; cancel: [] }>()

const COLOR_PRESETS = ['#9adcff', '#8b5cf6', '#f7f7f7', '#e100ff', '#61dca3', '#fb7185']
const sliders = [
  { key: 'speed', label: 'Speed', min: 0, max: 2, step: 0.05, digits: 2 },
  { key: 'scale', label: 'Scale', min: 0.5, max: 4, step: 0.1, digits: 1 },
  { key: 'brightness', label: 'Brightness', min: 0.2, max: 2, step: 0.05, digits: 2 },
  { key: 'bandHeight', label: 'Band Height', min: 0.1, max: 0.9, step: 0.05, digits: 2 },
  { key: 'colorSpeed', label: 'Color Speed', min: 0, max: 3, step: 0.1, digits: 1 },
] as const
const colors = [
  { key: 'color1', label: '色層 1' },
  { key: 'color2', label: '色層 2' },
] as const

const copied = ref(false)
const copyProps = () => {
  const c = props.config
  const text = `<KnowledgeSoftAurora
  :speed="${c.speed}"
  :scale="${c.scale}"
  :brightness="${c.brightness}"
  :band-height="${c.bandHeight}"
  :color-speed="${c.colorSpeed}"
  color1="${c.color1}"
  color2="${c.color2}"
