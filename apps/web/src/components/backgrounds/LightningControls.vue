...
const props = defineProps<{ config: LightningConfig; closable?: boolean }>()
const emit = defineEmits<{ done: []; cancel: [] }>()

const sliders = [
  { key: 'hue', label: 'Hue', min: 0, max: 360, step: 1, digits: 0 },
  { key: 'xOffset', label: 'X Offset', min: -1, max: 1, step: 0.05, digits: 2 },
  { key: 'speed', label: 'Speed', min: 0, max: 3, step: 0.1, digits: 1 },
  { key: 'intensity', label: 'Intensity', min: 0, max: 3, step: 0.1, digits: 1 },
  { key: 'size', label: 'Size', min: 0.3, max: 3, step: 0.1, digits: 1 },
] as const

const copied = ref(false)
const copyProps = () => {
  const c = props.config
  const text = `<KnowledgeLightning
  :hue="${c.hue}"
  :x-offset="${c.xOffset}"
  :speed="${c.speed}"
