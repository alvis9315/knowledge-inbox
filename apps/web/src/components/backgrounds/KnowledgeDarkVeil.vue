...
import DarkVeilControls from './DarkVeilControls.vue'

// React Bits "DarkVeil" ported verbatim to Vue 3(OGL + 官方 CPPN GLSL:
// 神經網路權重直接烘進 shader 的暗色有機流動紋理,可調色相/掃描線/噪點/扭曲)。
...
const cfg = reactive<DarkVeilConfig>({
...
const renderFrame = () => {
  ...
  program.uniforms.uNoise.value = cfg.noiseIntensity
  program.uniforms.uScan.value = cfg.scanlineIntensity
  program.uniforms.uScanFreq.value = cfg.scanlineFrequency
  program.uniforms.uWarp.value = cfg.warpAmount
...
onMounted(() => {
  ...
  if (!c || !cv) return
  renderer = new Renderer({ dpr: Math.min(window.devicePixelRatio || 1, 2), canvas: cv })
  ...
  program = new Program(gl, {
    ...
    uniforms: {
      ...
      uNoise: { value: cfg.noiseIntensity },
      uScan: { value: cfg.scanlineIntensity },
      uScanFreq: { value: cfg.scanlineFrequency },
      uWarp: { value: cfg.warpAmount },
    },
  })
...
<template>
  <div ref="containerRef" class="knowledge-darkveil" :class="props.class" aria-hidden="true">
    <canvas ref="canvasRef" class="block h-full w-full" />
    <DarkVeilControls v-if="showControls" :config="cfg" closable @done="ctlDone" @cancel="ctlCancel" />
  </div>
</template>
