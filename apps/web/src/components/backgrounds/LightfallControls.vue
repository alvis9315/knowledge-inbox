...
import type { LightfallConfig } from './KnowledgeLightfall.vue'
import BgPresetTabs from '@/features/theme/BgPresetTabs.vue'

// KnowledgeLightfall 的控制面板(Speed/Scale/Brightness/BandHeight/ColorSpeed
// + 雙色 + 互動),直接編輯傳入的 reactive config。模式同 ThreadsControls。
const props = defineProps<{ config: LightfallConfig; closable?: boolean }>()
const emit = defineEmits<{ done: []; cancel: [] }>()

const COLOR_PRESETS = ['#A6C8FF', '#5227FF', '#FF9FFC', '#61dca3', '#f4c430', '#0a1030']
const sliders = [
  { key: 'speed', label: 'Speed', min: 0, max: 2, step: 0.05, digits: 2 },
  { key: 'streakCount', label: 'Streaks', min: 1, max: 16, step: 1, digits: 0 },
  { key: 'glow', label: 'Glow', min: 0, max: 3, step: 0.1, digits: 1 },
  { key: 'density', label: 'Density', min: 0.1, max: 2, step: 0.05, digits: 2 },
  { key: 'zoom', label: 'Zoom', min: 1, max: 8, step: 0.5, digits: 1 },
...
const copyProps = () => {
  ...
  const text = `<KnowledgeLightfall
  :speed="${c.speed}"
  :streak-count="${c.streakCount}"
  :glow="${c.glow}"
  :density="${c.density}"
  :zoom="${c.zoom}"
  :opacity="${c.opacity}"
