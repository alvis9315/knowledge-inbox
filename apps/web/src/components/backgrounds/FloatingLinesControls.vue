...
const props = defineProps<{ config: FloatingLinesConfig; closable?: boolean }>()
const emit = defineEmits<{ done: []; cancel: [] }>()

const COLOR_PRESETS = ['#4a5f9e', '#8a6fc8', '#c86fb8', '#61dca3', '#5227FF', '#f4c430']
const sliders = [
  { key: 'animationSpeed', label: 'Speed', min: 0.1, max: 3, step: 0.1, digits: 1 },
  { key: 'lineCount', label: 'Lines', min: 2, max: 16, step: 1, digits: 0 },
  { key: 'lineDistance', label: 'Distance', min: 1, max: 12, step: 0.5, digits: 1 },
  { key: 'bendRadius', label: 'Bend Radius', min: 1, max: 12, step: 0.5, digits: 1 },
  { key: 'bendStrength', label: 'Bend Strength', min: -2, max: 2, step: 0.1, digits: 1 },
] as const
const colors = [
...
const copyProps = () => {
  ...
  const text = `<KnowledgeFloatingLines
  :animation-speed="${c.animationSpeed}"
  :line-count="${c.lineCount}"
  :line-distance="${c.lineDistance}"
  :bend-radius="${c.bendRadius}"
  :bend-strength="${c.bendStrength}"
  color1="${c.color1}"
  color2="${c.color2}"
  color3="${c.color3}"
