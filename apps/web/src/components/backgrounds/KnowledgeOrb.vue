...
import OrbControls from './OrbControls.vue'

// React Bits "Orb" ported verbatim to Vue 3(OGL + 官方 GLSL:
// simplex noise 光暈能量球,hue 可調、hover 扭曲+旋轉)。
...
const cfg = reactive<OrbConfig>({
  hue: props.hue,
  hoverIntensity: props.hoverIntensity,
  rotateOnHover: props.rotateOnHover,
...
const resize = () => {
  ...
  renderer.setSize(c.clientWidth * dpr, c.clientHeight * dpr)
  gl.canvas.style.width = `${c.clientWidth}px`
  gl.canvas.style.height = `${c.clientHeight}px`
...
const renderFrame = (t: number) => {
  ...
  program.uniforms.hue.value = cfg.hue
  program.uniforms.hoverIntensity.value = cfg.hoverIntensity

  const effectiveHover = props.forceHoverState ? 1 : cfg.enableMouseInteraction ? targetHover : 0
  program.uniforms.hover.value += (effectiveHover - program.uniforms.hover.value) * 0.1

  if (cfg.rotateOnHover && effectiveHover > 0.5) currentRot += dt * ROTATION_SPEED
...
onMounted(() => {
  ...
  program = new Program(gl, {
    ...
    uniforms: {
      ...
      hover: { value: 0 },
      rot: { value: 0 },
      hoverIntensity: { value: cfg.hoverIntensity },
...
<template>
  <div ref="containerRef" class="knowledge-orb" :class="props.class" aria-hidden="true">
    <OrbControls v-if="showControls" :config="cfg" closable @done="ctlDone" @cancel="ctlCancel" />
  </div>
</template>
