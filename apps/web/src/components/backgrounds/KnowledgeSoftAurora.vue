...
import SoftAuroraControls from './SoftAuroraControls.vue'

// React Bits "SoftAurora" ported verbatim to Vue 3(OGL + 官方 GLSL:
// 三倍頻 perlin3D 的柔霧極光帶,雙色層疊 + cosine gradient 流色)。
// 內建控制面板(show-controls),模式同 KnowledgeThreads。
export interface SoftAuroraConfig {
  speed: number
  scale: number
  brightness: number
  color1: string
...
const cfg = reactive<SoftAuroraConfig>({
  speed: props.speed,
  scale: props.scale,
  brightness: props.brightness,
  color1: props.color1,
  color2: props.color2,
  bandHeight: props.bandHeight,
  colorSpeed: props.colorSpeed,
...
const renderFrame = (t: number) => {
  ...
  program.uniforms.uBandHeight.value = cfg.bandHeight
  program.uniforms.uColorSpeed.value = cfg.colorSpeed
...
<template>
  <div ref="containerRef" class="knowledge-softaurora" :class="props.class" aria-hidden="true">
    <SoftAuroraControls v-if="showControls" :config="cfg" closable @done="ctlDone" @cancel="ctlCancel" />
  </div>
</template>

<style scoped>
.knowledge-softaurora {
  position: absolute;
  inset: 0;
  overflow: hidden;
  pointer-events: none;
  background: linear-gradient(180deg, #060911 0%, #0a0e1c 60%, #080a14 100%);
}
.knowledge-softaurora :deep(canvas) {
  display: block;
  width: 100%;
  height: 100%;
}
