...
import type { LightPillarConfig } from './KnowledgeLightPillar.vue'

// KnowledgeLightPillar 的控制面板(Speed/Scale/Brightness/BandHeight/ColorSpeed
// + 雙色 + 互動),直接編輯傳入的 reactive config。模式同 ThreadsControls。
const props = defineProps<{ config: LightPillarConfig; closable?: boolean }>()
const emit = defineEmits<{ done: []; cancel: [] }>()

const COLOR_PRESETS = ['#5227FF', '#FF9FFC', '#9adcff', '#61dca3', '#f4c430', '#fb7185']
const sliders = [
  { key: 'intensity', label: 'Intensity', min: 0.2, max: 3, step: 0.1, digits: 1 },
  { key: 'rotationSpeed', label: 'Rotation', min: 0, max: 2, step: 0.05, digits: 2 },
  { key: 'pillarWidth', label: 'Width', min: 1, max: 6, step: 0.2, digits: 1 },
  { key: 'pillarHeight', label: 'Height', min: 0.1, max: 1, step: 0.05, digits: 2 },
  { key: 'noiseIntensity', label: 'Noise', min: 0, max: 1.5, step: 0.05, digits: 2 },
  { key: 'pillarRotation', label: 'Tilt', min: -90, max: 90, step: 5, digits: 0 },
] as const
const colors = [
  { key: 'topColor', label: '頂色' },
  { key: 'bottomColor', label: '底色' },
] as const

const copied = ref(false)
const copyProps = () => {
  const c = props.config
  const text = `<KnowledgeLightPillar
  top-color="${c.topColor}"
  bottom-color="${c.bottomColor}"
  :intensity="${c.intensity}"
  :rotation-speed="${c.rotationSpeed}"
  :pillar-width="${c.pillarWidth}"
  :pillar-height="${c.pillarHeight}"
  :noise-intensity="${c.noiseIntensity}"
  :pillar-rotation="${c.pillarRotation}"
...
<template>
  <div class="lightpillar-controls">
